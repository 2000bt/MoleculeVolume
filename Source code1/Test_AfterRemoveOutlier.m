...
if ~isempty(blackPoints)
    %scatter3(blackPoints(:, 1), blackPoints(:, 2), blackPoints(:, 3), 10, 'k', 'filled');
    blackPoints = blackPoints(blackPoints(:, 1) >= min_x_green & blackPoints(:, 1) <= max_x_green & ...
                                    blackPoints(:, 2) >= min_y_green & blackPoints(:, 2) <= max_y_green & ...
                                    blackPoints(:, 3) >= min_z_green & blackPoints(:, 3) <= max_z_green, :);
    scatter3(blackPoints(:, 1), blackPoints(:, 2), blackPoints(:, 3), pointSize1, 'k', 'filled', 'MarkerFaceAlpha', 0.4);
    hold on;
end
