  ...
  switch lower(algorithm)
    case 'moller'
      ...
      [intMatrix(tMsk), intSurface] = TriangleIntersection3D_Moller(...
        ...
        face1,face2,getIntersection, debug);
    case 'rapid'
...
if nnz(tMsk)>0
  ...
  if getIntersection
    ...
    intSurface.triangles    = [intSurface.triangles;    intSurface2.triangles];
    if debug
      np = size(intSurface.vertices,1);
      assert(max(intSurface.faces(:))<=np, 'Bad surface definition')
      assert(max(intSurface.edges(:))<=np, 'Bad surface definition')
      assert(max(intSurface.triangles(:))<=np, 'Bad surface definition')
    end
  end
end
...
%   intSurface.triangles = ic(intSurface.triangles);
end
end % function
