function [mLineSub_cell,LineFlag,vRectangle,iCountContour] = Func_Unicursal_5(vXTemp, vYTemp, dSlice)
    figure;
    hold on;
 ...
 %% 一次轮廓搜索       
        while(iCountUnUsed > 0)
            ...
            iNext = vIdxUU(iMin);  % 取得最小距离点的索引
            mLine(iCount, 2) = iNext;    

            % 画线，连接当前点与下一个最小距离点
            line([vXTemp(iLeft), vXTemp(iNext)], [vYTemp(iLeft), vYTemp(iNext)], 'Color', vColor(iCountContour), 'LineWidth', 2);
            if(iNext==121)
                 plot(vXTemp(iNext),vYTemp(iNext),'ro');
            end
            % 更新状态
            vUsed(iNext) = 1;  % 标记为已使用
            iCount = iCount + 1;  % 更新轮廓线的编号
            iLeft = iNext;  % 将当前位置更新为当前点
            mLine(iCount, 1) = iLeft;  % 保存当前位置索引
