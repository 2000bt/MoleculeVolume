function [mLineSub_cell,LineFlag,vRectangle,iCountContour] = Func_Unicursal_TwoSearch1(vXTemp, vYTemp, dSlice)
    figure;
    hold on;
    iLen = length(vXTemp);
...
%          if iCountUnUsed
        ...
        while(iCountUnUsed > 0)
            ...
            iMin = iMin(1);  % 如果有多个最小值，只取第一个
            iTemp = vIdxUU(iMin);
            iTemp_Dis = (vXTemp(iTemp) - vXTemp(iRight))^2 + (vYTemp(iTemp) - vYTemp(iRight))^2;
            vUsed(iTemp) = 1;
%             if(iTemp==122)
%                 a=1;
%             end
