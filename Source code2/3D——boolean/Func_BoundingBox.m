function [vdx,vdx1]=Func_BoundingBox(vX, vY, vZ, vFace,AtomNum,Atom_info,vX1, vY1, vZ1,vFace1,AtomNum1,Atom_info1)
%     iCount = 0;
%     iCount1 = 0;
num=zeros(1,4);
%% AABB包围盒先点后面
...
%     hold off; 
    ...
    num(1,3)=num(1,1)-length(vdx);
    vdx1 = vdx1_AABB(vdx1);  % 还原原始 vFace1 的
    num(1,4)=num(1,2)-length(vdx1);
function matching_rows = findOverlappingFaces(vX, vY, vZ, vFace, Atom_info1, len)
    ...
    for i = 1 : size(Atom_info1, 1) % Atom_info1 的行数是原子数目
        ...
        atom_radius = 1.3*2.2110 * Atom_info1(i, 5); % 原子的半径
